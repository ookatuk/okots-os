...
impl HybridAllocator {
    pub fn new(data_list: Vec<MemData>) -> Self {
...
impl HybridAllocatorInner {
    ...
    unsafe fn dealloc_from_frames(&mut self, ptr: *mut u8, layout: Layout) {
        ...
        for alloc_mutex in &mut self.frame_allocs {
            ...
            let start_page = alloc.table_ptr.start.as_ptr().read() as usize;
...
unsafe impl GlobalAlloc for HybridAllocator {
    unsafe fn alloc(&self, layout: Layout) -> *mut u8 {
        ...
        if size > 2048 || layout.align() > 4096 {
            return mgr.alloc_from_frames(layout);
        }
        ...
        if let Some(mut current) = mgr.slab_heads[bucket] {
        ...
        let new_page = mgr.alloc_from_frames(Layout::from_size_align_unchecked(4096, 4096));
        if new_page.is_null() { return null_mut(); }

        // 3. 新しいスラブを初期化し、リストの先頭に挿入
        // 以前の先頭(mgr.slab_heads[bucket])を next に渡す
        let new_slab = InternalSlab::init_at(new_page, slot_size as u32, mgr.slab_heads[bucket]);
        let new_slab_ptr = NonNull::new_unchecked(new_slab);

        // Vec の中身を更新
        mgr.slab_heads[bucket] = Some(new_slab_ptr);

        new_slab.alloc(layout).unwrap_or(null_mut())
    }
    ...
    unsafe fn dealloc(&self, ptr: *mut u8, layout: Layout) {
        ...
        if layout.size() > 2048 || layout.align() > 4096 {
            // 【重要】どのアロケータに属するか探して返却
            mgr.dealloc_from_frames(ptr, layout);
        } else {
            let slab_addr = (ptr as usize) & !4095;
            let slab = &mut *(slab_addr as *mut InternalSlab);
            slab.dealloc(ptr);
        }
    }
    ...
    unsafe fn alloc_zeroed(&self, layout: Layout) -> *mut u8 {
        ...
        if size > 2048 || layout.align() > 4096 {
            ...
            for alloc_mutex in &mut mgr.frame_allocs {
                let ptr = alloc_mutex.lock().alloc_zeroed(layout);
        ...
        if old_size > 2048 && new_size > 2048 {
            ...
            for alloc_mutex in &mut mgr.frame_allocs {
                ...
                if addr >= (start_page << 12) && addr < (start_page << 12) + (u32::MAX as usize) {
                    return alloc.realloc(ptr, layout, new_size);
                }
            }
        }
